...
BEGIN
    ...
    BEGIN
        PRINT 'Tên hãng s?n xu?t ?ã t?n t?i. Vui lòng ki?m tra l?i!'
    END
    ELSE
    BEGIN
...
BEGIN
    ...
    END
END
EXEC sp_ThemSuaSanPham 'SP01', 'H01', 'Galaxy Note 11', '50', '??','1900000.00','Chi?c','Hàng c?n c?o c?p'
...
BEGIN
    ...
    BEGIN
        PRINT 'Hãng không t?n t?i trong b?ng'
        RETURN
    END
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            RAISERROR('Mã nhân viên ?ã t?n t?i!', 16, 1);
            RETURN;
        END
...
BEGIN
    ...
    BEGIN
        PRINT 'Mã s?n ph?m không t?n t?i'
        RETURN
    END
    IF NOT EXISTS(SELECT * FROM Nhanvien WHERE manv = @manv)
    BEGIN
        PRINT 'Mã nhân viên không t?n t?i'
        RETURN
    END
...
BEGIN
    ...
    IF NOT EXISTS (SELECT * FROM Sanpham WHERE masp = @masp)
    BEGIN
        PRINT 'Mã s?n ph?m không t?n t?i trong b?ng Sanpham.'
        RETURN
    END
    ...
    IF NOT EXISTS (SELECT * FROM Nhanvien WHERE manv = @manv)
    BEGIN
        PRINT 'Mã nhân viên không t?n t?i trong b?ng Nhanvien.'
        RETURN
    END
    ...
    BEGIN
        PRINT 'S? l??ng xu?t v??t quá s? l??ng t?n kho.'
        RETURN
    END
    ...
    BEGIN
        ...
        WHERE sohdx = @sohdx
        PRINT 'C?p nh?t d? li?u b?ng Xuat thành công.'
    END
    ELSE
    BEGIN
        INSERT INTO Xuat(sohdx, masp, manv, ngayxuat, soluongX)
        VALUES (@sohdx, @masp, @manv, @ngayxuat, @soluongX)
        PRINT 'Thêm d? li?u vào b?ng Xuat thành công.'
    END
END
...
BEGIN
    ...
    IF NOT EXISTS(SELECT * FROM Nhanvien WHERE manv = @manv)
    BEGIN
        PRINT 'Không tìm th?y nhân viên v?i mã ' + CAST(@manv AS NVARCHAR)
        RETURN
    END
    ...
    PRINT '?ã xóa nhân viên v?i mã ' + CAST(@manv AS NVARCHAR)
END
...
BEGIN
  ...
  BEGIN
    PRINT 'Không tìm th?y s?n ph?m ?? xóa!'
    RETURN;
  END
  ...
  BEGIN TRY
    ...
    COMMIT TRANSACTION
    PRINT '?ã xóa s?n ph?m ' + @masp
  END TRY
  BEGIN CATCH
    ROLLBACK TRANSACTION
    PRINT '?ã x?y ra l?i trong quá trình xóa s?n ph?m!'
  END CATCH
END